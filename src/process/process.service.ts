...
export class ProcessService {
  ...
  totalCurrentSells = async () => {
    ...
    try {
      ...
      const filterOrders = allOrders.filter(
        (order) => order.status != CANCELLED_STATUS && order.status === FINISHED_STATUS,
      );
  ...
  totalPeriodSells = async (id: string) => {
    ...
    try {
      ...
      const filterOrders = allOrders.filter((order) => order.status === FINISHED_STATUS);
      ...
      const debugTotalPayments = filterOrders.reduce((acc, order) => {
      ...
      const debugTotalsMessage = `Calculo legacy = ${debugTotalPayments}, Calculo por transacciones = ${sellTotal} `;
      await this.sendMessageService.SendTelegramMessage(debugTotalsMessage);

      await session.commitTransaction();
      session.endSession();
      const counterSells = allOrders.length ?? 0;

      return { totalSellCount: counterSells, totalSellAmount: sellTotal, debugTotalsMessage };
    } catch (error) {
      await session.abortTransaction();
      session.endSession();
      throw error;
  ...
  especifTotalPeriodSells = async (id: string, type: string) => {
    ...
    try {
      ...
      const filterCancelledOrders = allOrders.filter((order) => order.status !== CANCELLED_STATUS);
  ...
  especificSellsForPayment = async (id: string, type?: string) => {
    ...
    try {
      /* Vamos a dejar esto asi, pero probablemente se tengan que filtrar 
          ...
          cuentas filtradas por su tipo de venta y no por si tenga pagos o no. 
      ...
      const filterTransactions = allTransactions.filter((transaction) =>
        type
          ...
          : ['cash', 'credit', 'debit', 'transfer'].includes(transaction.paymentType),
      );
