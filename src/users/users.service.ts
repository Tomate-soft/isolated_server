...
export class UsersService {
  ...
  async create(createUser: CreateUserDto) {
    try {
      const lastUser = await this.UserModel.findOne({}).sort({ employeeNumber: -1 }).exec();
      ...
      const userToCreate = new this.UserModel({
        ...createUser,
        employeeNumber: createUser.employeeNumber ? createUser.employeeNumber : nextEmployeeNumber,
      });
  ...
  async resetDailyRegister() {
    return await this.UserModel.updateMany({}, { $set: { dailyRegister: null } });
  }
