...
export class CancellationReasonController {
  ...
  async delete(@Param('id') id: string) {
    try {
      ...
      if (!deletedReason) {
        throw new NotFoundException('No se encontro el motivo que deseas eliminar');
      }
  ...
  @Put(':id')
  async update(@Param('id') id: string, @Body() body: UpdateCancellationReasonDto) {
    try {
      const updatedReason = await this.cancellationReasonService.update(id, body);
      if (!updatedReason) {
        throw new NotFoundException('No se encontro el motivo a actualizar');
      }
      return updatedReason;
    } catch (error) {
      throw new NotFoundException('No se pudo actualizar, debido a un error inesperado');
    }
  }
}
