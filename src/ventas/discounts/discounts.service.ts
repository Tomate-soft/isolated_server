...
export class DiscountsService {
      ...
      switch (payload.body.discountType) {
        ...
        case PRODUCTS_DISCOUNTS:
          if (payload.accountApt.noteNumber) {
            const newDiscountNote = await this.discountModel.create(createDiscountData);
            if (!newDiscountNote) {
            ...
            const newProductsForBill = currentBillNote.notes.flatMap((element) => element.products);

            const newBillTotal = calculateBillTotal(newProductsForBill);

            const aptBill = await this.billsModel.findByIdAndUpdate(currentBillNote._id, {
              checkTotal: newBillTotal,
              products: newProductsForBill,
            });
            return updatedNote;
          }
        ...
        case NOTES_DISCOUNTS:
          ...
          const updateNote = await this.noteModel.findByIdAndUpdate(newDiscountNote.noteAccountId, {
        ...
        case BILL_DISCOUNTS:
          const newDiscountBill = await this.discountModel.create(createDiscountData);
          ...
          if (payload.body.discountType === COURTESY_APPLY_BILL) {
            const updateTable = await this.tableModel.findByIdAndUpdate(updatedBillDiscount.table, {
              status: FOR_PAYMENT_STATUS,
            });
            const currentPeriod: any = await this.operatingPeriodService.getCurrent();
            const randomIndex =
              currentPeriod[0].sellProcess.length <= 1
                ? 0
                : Math.floor(Math.random() * currentPeriod[0].sellProcess.length);
            const cashierSessionId = currentPeriod[0].sellProcess[randomIndex]._id; // chequemos cada parte de esto
            const selectSession = await this.cashierSessionModel.findById(cashierSessionId);
            const updatedSession = await this.cashierSessionModel.findByIdAndUpdate(
              cashierSessionId,
              {
        ...
        case COURTESY_APPLY_NOTES:
          const newCourtesyNote = await this.discountModel.create(createDiscountData);
          ...
          const enableNotes = courtesyBill.notes.filter((note) => note.status === ENABLE_STATUS);
          ...
          if (courtesyBill.cashierSession) {
            ...
            if (enableNotes.length <= 0) {
              ...
              const tableUpdated = await this.tableModel.findByIdAndUpdate(courtesyBill.table, {
                status: FOR_PAYMENT_STATUS,
              });
            }
          ...
          const randomIndex = Math.floor(Math.random() * currentPeriod[0].sellProcess.length);

          const cashierSessionId = currentPeriod[0].sellProcess[randomIndex]._id;
  ...
  async deleteDiscountProductInNote(id: string, body: updateNoteDto) {
    ...
    try {
      ...
      const noteToBillSendProducts = currentBill.notes.flatMap((element) => element.products);
