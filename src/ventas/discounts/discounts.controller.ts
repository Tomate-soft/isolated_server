...
export class DiscountsController {
  ...
  async deleteDiscount(@Param('id') id: string, @Body() body: any) {
    try {
      const updatedDiscount = await this.discountService.deleteDiscounte(id, body);
      if (!updatedDiscount) {
        throw new NotFoundException('No se encuentra el descuento que se desea actualizar');
      }
      return updatedDiscount;
  ...
  @Put('d/note/:id')
  async deleteDiscountProductInNote(@Param('id') id: string, @Body() body: any) {
    try {
      const updateDiscount = await this.discountService.deleteDiscountProductInNote(id, body);
      if (!updateDiscount) {
  ...
  @Put('d/bill/:id')
  async deleteDiscountProductInBill(@Param('id') id: string, @Body() body: any) {
    try {
      const updateDiscount = await this.discountService.deleteDiscountProductInBill(id, body);
      if (!updateDiscount) {
        throw new NotFoundException('No se puso completar');
      }
      return updateDiscount;
