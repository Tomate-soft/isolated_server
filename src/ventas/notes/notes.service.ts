...
export class NotesService {
  ...
  async create(createNote: createNoteDto) {
    const lastNote = await this.noteModel
      ...
      .sort({ createdAt: -1 })
      .exec();
    const nextNoteCode = lastNote ? this.getNextNoteCode(lastNote.noteNumber) : 1;
    ...
    if (accountId && accountId != null) {
      if (updatedNote.status) {
        ...
        try {
          ...
          const searchAccount = await this.BillsModel.findById(accountId).populate({
            path: 'notes',
          });
          if (searchAccount.cashierSession) {
            ...
            if (pendingNotes.length === 0) {
              ...
              const table = await this.tableModel.findByIdAndUpdate(searchAccount.table, {
                status: 'forPayment',
              });
            }
          ...
          // );
          const cashierSessionId = currentPeriod[0].sellProcess[0]._id;
      ...
      if (billCurrent && billCurrent.notes.length > 0) {
        ...
        if (upNote) {
          const refreshedBill = await this.BillsModel.findById(accountId).populate({
            path: 'notes',
          });

          const sumProducts = [...refreshedBill.notes].flatMap((nota) => nota.products);
