...
export class SellTypesController {
  ...
  async delete(@Param('id') id: string) {
    try {
      ...
      if (!sellTypeDeleted) {
        throw new NotFoundException('No se ha encontrado el turno que deseas eliminar');
      }
  ...
  async update(@Param('id') id: string, @Body() body: UpdateSellTypeDto) {
    try {
      ...
      if (!updatedSellType) {
        throw new NotFoundException('No se encontro el turno que deseas modificar');
      }
      return updatedSellType;
    } catch (error) {
      throw new NotFoundException('No se pudo actualizar el turno debido a un error inesperado');
    }
  }
}
