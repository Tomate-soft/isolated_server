...
*/
  ...
  async createBranch(branch: any) {
    try {
      ...
      const business = await this.businessModel.findByIdAndUpdate(branch.businessId, {
  ...
  async getBusinesses(licenseId: string) {
    const businesses = await this.businessModel
      .find({ licenseKey: new Types.ObjectId(licenseId) })
      .populate({
        path: 'branches',
        populate: {
          path: 'devices',
          populate: {
            path: 'settings',
            populate: { path: 'printers' },
          },
        },
      })
      .populate('licenseKey') // 👈 ahora sí puedes traer también el objeto completo
      .lean()
      .exec();
  ...
  async createLicenseKey(bussinesId: { bussinesId: string }) {
    try {
      ...
      const currentBusiness = await this.businessModel.findByIdAndUpdate(bussinesId.bussinesId, {
