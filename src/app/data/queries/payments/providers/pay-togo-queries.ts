...
export class PayTogoQueries {
  ...
  async payTogoOrder(id: string, body: any): Promise<any> {
    const todayPrefix = `TG${toCompactDate(new Date())}`;
    const response: {
      order: ToGoOrder;
      payment: Payment;
    } = {} as { order: ToGoOrder; payment: Payment };
    await this.connection.transaction(async (session) => {
      const order = await this.toGoOrderModel.findOneAndUpdate(
      ...
      await this.toGoOrderModel.findByIdAndUpdate(id, updateBillData, { session });
