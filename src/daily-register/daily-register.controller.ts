...
export class DailyRegisterController {
  ...
  async create(@Body() body: CreateDailyRegisterDto) {
    try {
      ...
      if (error.code === 11000) {
        throw new ConflictException(`No se pudo completar el registro ${error}`);
      }
  ...
  @Put(':id')
  async updateRegister(@Param('id') id: string, @Body() body: UpdateDailyRegisterDto) {
    try {
      const updateRegister = await this.dailyRegisterService.updateRegister(id, body);
