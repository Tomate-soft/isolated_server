...
export class TablesService {
  ...
  async joinTables(body: any) {
    ...
    const tableWithAccount = tablesCopy.filter((table) => table.bill.length > 0);
    ...
    if (accountEnable) {
      ...
      const tablesNoActive = tablesCopy.filter((table) => table._id !== tableActive._id);
      ...
      const tableMin = tablesCopy.reduce(
        (min, table) => (parseFloat(table.tableNum) < parseFloat(min.tableNum) ? table : min),
        tablesCopy[0],
      );

      // Filtrar las mesas restantes que no son la mínima
      const tablesNoMin = tablesCopy.filter((table) => table._id !== tableMin._id);
  ...
  */
  ...
  async separateTables(id: string) {
    ...
    try {
      ...
      await this.tableModel.findByIdAndUpdate(id, { joinedTables: [] }, { session });
      await session.commitTransaction();
      session.endSession();
