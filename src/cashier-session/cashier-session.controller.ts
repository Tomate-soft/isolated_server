...
export class CashierSessionController {
  ...
  async delete(@Param('id') id: string) {
    try {
      ...
      throw new NotFoundException(` Ha ocurrido algo inesperado, Mas información: ${error}`);
    }
  }
  ...
  async update(@Param('id') id: string, @Body() body: updateCashierSessionDto) {
    try {
      ...
      if (!updatedSession) {
        throw new NotFoundException(`No se encontro la session que deseas actualizar`);
      }
  ...
  async create(@Body() body: createCashierSessionDto) {
    try {
      ...
      if (error.code === 11000) {
        throw new ConflictException(`Esta se session ya se encuentra activa. ${error}`);
      }
      throw new NotFoundException(`Ha ocurrido algo inesperado. Mas informacion: ${error}`);
    }
  }
  ...
  @Put('payment/:id')
  async updateBillForPayment(@Param('id') id: string, @Body() body: updateCashierSessionDto) {
    try {
      const updatedData = await this.cashierSessionService.updateBillForPayment(id, body);
      if (!updatedData) {
        throw new NotFoundException('No se pudo actualizar');
      }
      return updatedData;
    } catch (error) {
      throw new NotFoundException(`Ha ocurrido un error inepserado, mas informacion: ${error}`);
    }
  }

  @Post('/cash-withdrawal')
  async cashWithdrawal(@Body() body: { auth: any; body: createCashWithdrawDto }) {
    try {
