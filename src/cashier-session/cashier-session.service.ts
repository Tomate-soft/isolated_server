...
export class CashierSessionService {
  ...
  async create(body: createCashierSessionDto) {
    ...
    const updatedOperatingPeriod = await this.operatingPeriodModel.findOne().sort({ _id: -1 });

    const data = {
      ...body,
      startDate: new Date().toISOString(),
      operatingPeriod: updatedOperatingPeriod._id,
    };
    ...
    if (newSession._id) {
      try {
        ...
        throw new NotFoundException('No se completo la actualizacion de usuario');
      }
    } //
  ...
  async updateBillForPayment(id: string, body: updateCashierSessionDto) {
    ...
    try {
      ...
      */
      ...
      session.endSession();
      console.error(`Hubo un error inesperado surante la sesion, mas informacion: ${error}`);
    }
  }
