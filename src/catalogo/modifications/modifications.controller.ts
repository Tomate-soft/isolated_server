...
export class ModificationsController {
  ...
  async findOne(id: string) {
    try {
      ...
      if (!selectedModifier)
        throw new NotFoundException('El modificador que intentas buscar no existe');
  ...
  @Put(':id')
  async update(@Param('id') id: string, @Body() updatedModifier: updateModifierDto) {
    console.log(updatedModifier);
    console.log(id);
    try {
      const modifierUpdated = await this.modifierService.update(id, updatedModifier);
      if (!modifierUpdated) {
