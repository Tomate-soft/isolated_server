...
import { Prices, Products } from '../../schemas/catalogo/products.schema';
...
export class ProductsService {
  ...
  async findAllWithRestaurantPrice(sellType: string): Promise<any[]> {
    const filter = this.getFilterBySellType(sellType);
    ...
    const formatProducts = productsArray?.map((product) => {
      const newPrices = product?.prices?.filter((price) => price.name === filter);
      return {
        ...product,
        prices: newPrices,
      };
  ...
  async update(id: string, updatedProduct: UpdateProductDto) {
    try {
      const currentProduct = await this.productsModel.findByIdAndUpdate(id, updatedProduct, {
        new: true,
      });
