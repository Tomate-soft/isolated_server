...
export class ProductsController {
  ...
  @Get('command/:sellType')
  async productsWithRestaurantPrice(@Param('sellType') sellType: string) {
    try {
      const productsArray = await this.productService.findAllWithRestaurantPrice(sellType);
      if (!productsArray || productsArray.length === 0)
        throw new NotFoundException('No se encontraron productos');
      return productsArray;
    } catch (error) {
  ...
  async findOne(@Body() body: string) {
    try {
      ...
      if (!selectedProduct)
        throw new ConflictException('El producto que intentas encontrar no existe');
      return;
    } catch (error) {}
  }
