...
export class SettingController {
  ...
  async findAll() {
    try {
      ...
      if (!settingsArray) {
        throw new NotFoundException(`No se encontraron dispositivos disponibles`);
      }
  ...
  async findOne(@Param('id') id: string) {
    try {
      ...
      if (!settingSelected) {
        throw new NotFoundException(`El dispositivo ${id}, no se encuentra disponible o no existe`);
      }
      return settingSelected;
    } catch (error) {
      throw new NotFoundException(`Ha ocurrido un error inesperado en la busqueda ${error}`);
    }
  }
  ...
  @Put('printers/:id')
  async updatePrinters(@Param('id') id: string, @Body() body: UpdatePrinterDto) {
    try {
