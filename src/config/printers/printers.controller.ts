...
export class PrintersController {
  ...
  async findOne(@Param('id') id: string) {
    try {
      ...
      if (!selectedPrinter) {
        throw new NotFoundException(`No se encuentra la impresora con el id: ${id}`);
      }
  ...
  async delete(@Param('id') id: string) {
    try {
      ...
      if (!printerDeleted) {
        throw new NotFoundException(`No se encontro la impresora con le id: ${id}`);
      }
  ...
  async update(@Param('id') id: string, @Body() Body: UpdatePrinterDto) {
    ...
    try {
      ...
      if (!updatedPrinter) {
        throw new NotFoundException(`No se pudo actualizar la impresora: ${id}`);
      }
